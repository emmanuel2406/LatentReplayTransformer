...
from small_100.configuration_m2m_100 import M2M100Config
...
class LatentBaseModelOutput(BaseModelOutput):
    def __init__(self, latent_acts=None, **kwargs):
        super().__init__(**kwargs)
        self.latent_acts = latent_acts

class LatentSeq2SeqModelOutput(Seq2SeqModelOutput):
    def __init__(self, logits=None, latent_acts=None, **kwargs):
        super().__init__(**kwargs)
        self.latent_acts = latent_acts
        self.logits = logits
...
class M2M100Encoder(M2M100PreTrainedModel):
    ...
    def __init__(self, config: M2M100Config, embed_tokens: Optional[nn.Embedding] = None, latent_layer_num=6):
    ...
    def forward(
        ...
        latent_encoder_input: Optional[torch.Tensor] = None,
        latent_attention_mask: Optional[torch.Tensor] = None,
        return_lat_acts: Optional[bool] = False,
    ):
        r"""
        Args:
            input_ids (`torch.LongTensor` of shape `(batch_size, sequence_length)`):
            ...
            return_dict (`bool`, *optional*):
                Whether or not to return a [`~utils.ModelOutput`] instead of a plain tuple.
        """
        # print("DEBUG: Encoder forward begins")
        ...
        if attention_mask is not None:
            if self._use_flash_attention_2:
                ...
                latent_attention_mask = _prepare_4d_attention_mask_for_sdpa(latent_attention_mask, inputs_embeds.dtype) if latent_attention_mask is not None else None
            else:
                # [bsz, seq_len] -> [bsz, 1, tgt_seq_len, src_seq_len]
                attention_mask = _prepare_4d_attention_mask(attention_mask, inputs_embeds.dtype)
                latent_attention_mask = _prepare_4d_attention_mask(latent_attention_mask, inputs_embeds.dtype) if latent_attention_mask is not None else None
        ...
        for idx, encoder_layer in enumerate(self.layers):
            # print(f"DEBUG: encoder layer {idx}")
            if output_hidden_states: # WARNING: might need to only add original hidden states
                encoder_states = encoder_states + (hidden_states,)

            # Capture the latent activations at the specified layer
            if idx == self.latent_layer_num:
                latent_acts = hidden_states.clone().detach()

                # Inject latent replay activations if provided
                if latent_encoder_input is not None:
                    hidden_states = torch.cat((hidden_states, latent_encoder_input), dim=0)

                    if latent_attention_mask is not None:
                        attention_mask = torch.cat([attention_mask, latent_attention_mask], dim=0)
        ...
        if not return_dict:
            return tuple(v for v in [hidden_states, encoder_states, all_attentions, latent_acts] if v is not None)
        return LatentBaseModelOutput(
            last_hidden_state=hidden_states,
            hidden_states=encoder_states,
            attentions=all_attentions,
            latent_acts=latent_acts if return_lat_acts else None
        )
    ...
    def forward(
        ...
        Args:
            ...
            return_dict (`bool`, *optional*):
                Whether or not to return a [`~utils.ModelOutput`] instead of a plain tuple.
        """
        # print("DEBUG: Decoder forward begins")
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
...
class M2M100Model(M2M100PreTrainedModel):
    ...
    def __init__(self, config: M2M100Config, latent_layer_num: int):
        super().__init__(config)
        self.latent_layer_num = latent_layer_num
        ...
        self.encoder = M2M100Encoder(config, self.shared, latent_layer_num=latent_layer_num)
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        latent_encoder_input: Optional[torch.FloatTensor] = None,
        latent_attention_mask: Optional[torch.Tensor] = None,
        return_lat_acts: bool = False,
        ...
        if encoder_outputs is None:
            encoder_outputs = self.encoder(
                ...
                latent_encoder_input=latent_encoder_input,
                latent_attention_mask = latent_attention_mask,
                return_lat_acts=return_lat_acts,
            )
        ...
        latent_acts = encoder_outputs.latent_acts if return_lat_acts else None
        if latent_attention_mask is not None:
            attention_mask = torch.cat([attention_mask, latent_attention_mask], dim=0)

        # print(f"DEBUG: encoder_attention_mask shape: {attention_mask.shape}")
        ...
        if not return_dict:
            print("DEBUG: mistake, return_dict = False")
            return decoder_outputs + encoder_outputs

        lm_logits = self.lm_head(decoder_outputs.last_hidden_state)

        return LatentSeq2SeqModelOutput(
            logits=lm_logits,
...
def save_m2m100_model(model, save_path):
    """
    Saves the model's state_dict and config.

    Args:
        model (M2M100Model): The model instance to save.
    ...
    print(f"STORAGE: Model saved to {save_path}")

def load_m2m100_model(load_path, latent_layer_num):
    """
    Loads a model from the given directory path.

    Args:
        load_path (str): Directory path to load model from.
        latent_layer_num (int): The latent layer index used to initialize the model.

    Returns:
        model (M2M100Model): The loaded model instance.
    """
    # Load config
    config = M2M100Config.from_pretrained(load_path)
    # Instantiate model
    model = M2M100Model(config, latent_layer_num=latent_layer_num)
    # Load weights
    model.load_state_dict(torch.load(os.path.join(load_path, "pytorch_model.bin"), map_location="cpu"))
    print(f"STORAGE: Model loaded from {load_path}")
    return model
...
#     def __init__(self, config: M2M100Config, latent_layer_num: int):
#         super().__init__(config)
#         self.model = M2M100Model(config, latent_layer_num=latent_layer_num)
...
#             return_lat_acts = return_lat_acts,
#         )
#         latent_acts = outputs.latent_acts

#         print("DEBUG: outputs[0].shape=", outputs[0].shape)
#         print("DEBUG: latent_acts.shape=", latent_acts.shape)
